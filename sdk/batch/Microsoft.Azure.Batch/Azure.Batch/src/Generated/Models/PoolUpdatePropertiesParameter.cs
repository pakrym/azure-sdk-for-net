...
namespace Azure.Batch.Models
{
    ...
    public partial class PoolUpdatePropertiesParameter
    {
        ...
        public PoolUpdatePropertiesParameter(IEnumerable<CertificateReference> certificateReferences, IEnumerable<ApplicationPackageReference> applicationPackageReferences, IEnumerable<MetadataItem> metadata)
        {
            ...
            CertificateReferences = certificateReferences.ToArray();
            ApplicationPackageReferences = applicationPackageReferences.ToArray();
            Metadata = metadata.ToArray();
        }

        /// <summary> Initializes a new instance of PoolUpdatePropertiesParameter. </summary>
        /// <param name="startTask"> If this element is present, it overwrites any existing StartTask. If omitted, any existing StartTask is removed from the Pool. </param>
        /// <param name="certificateReferences"> This list replaces any existing Certificate references configured on the Pool. If you specify an empty collection, any existing Certificate references are removed from the Pool. For Windows Nodes, the Batch service installs the Certificates to the specified Certificate store and location. For Linux Compute Nodes, the Certificates are stored in a directory inside the Task working directory and an environment variable AZ_BATCH_CERTIFICATES_DIR is supplied to the Task to query for this location. For Certificates with visibility of &apos;remoteUser&apos;, a &apos;certs&apos; directory is created in the user&apos;s home directory (e.g., /home/{user-name}/certs) and Certificates are placed in that directory. </param>
        /// <param name="applicationPackageReferences"> The list replaces any existing Application Package references on the Pool. Changes to Application Package references affect all new Compute Nodes joining the Pool, but do not affect Compute Nodes that are already in the Pool until they are rebooted or reimaged. There is a maximum of 10 Application Package references on any given Pool. If omitted, or if you specify an empty collection, any existing Application Packages references are removed from the Pool. A maximum of 10 references may be specified on a given Pool. </param>
        /// <param name="metadata"> This list replaces any existing metadata configured on the Pool. If omitted, or if you specify an empty collection, any existing metadata is removed from the Pool. </param>
        internal PoolUpdatePropertiesParameter(StartTask startTask, IList<CertificateReference> certificateReferences, IList<ApplicationPackageReference> applicationPackageReferences, IList<MetadataItem> metadata)
        {
            StartTask = startTask;
